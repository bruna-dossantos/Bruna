...
Reusable Linear utility scripts. Copied 2026-08-14 from `~/Claude/Projects/Linear Scripts/` and **sanitized** — every hardcoded Linear token was replaced with `os.environ["LINEAR_TOKEN"]`.

## ⚠️ Before running
Set your token in the shell (do NOT hardcode it):
```bash
export LINEAR_TOKEN="lin_api_..."   # from ~/Claude/Projects/Credentials/linear_tokens.json
```
...
## Provenance
Originals remain in `~/Claude/Projects/Linear Scripts/` (with live tokens — see that folder's README and the rotation flag in `migration/ESTATE-MAP.md`).
