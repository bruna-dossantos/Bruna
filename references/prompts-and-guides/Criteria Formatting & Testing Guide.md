...
- Vague, open-ended
- Could give you history, breeds, care tips, or random facts
- Inconsistent results

**Good prompt:** "List 5 specific health benefits of dog ownership supported by medical research, with one sentence explaining each benefit"
- Specific output format (list of 5)
- Defined scope (health benefits, medical research)
- Clear structure (one sentence each)
...
### The Prompt Engineering You're Actually Doing

You're creating two types of prompts:

#### Type 1: Decision Logic Prompts (The "Rules")
...
> "The patient must have a mobility limitation that impacts their ability to perform at least one Mobility-Related Activity of Daily Living (MRADL) within the home, in the patient's current unassisted state. A qualifying mobility limitation means the patient is unable to perform the MRADL without assistance, OR attempting the MRADL without equipment places the patient at increased risk of illness, injury, or medical complications, OR the patient cannot complete the MRADL within a reasonable time frame without the prescribed device."

You've translated vague policy language into a specific decision tree the model can follow.

#### Type 2: Extraction Prompts (The "Search Instructions")
...
### The Three Core Prompt Engineering Skills

To write effective criteria, you need to master three prompt engineering skills:
...
|---------|--------------|-----|
| **Using policy language verbatim** | Written for humans, not AI | Translate into explicit decision logic |
| **Assuming shared knowledge** | Model doesn't know medical context | Define every term and relationship |
| **One-shot prompting** | First attempt rarely works | Iterate based on test results |
| **Ignoring the negative case** | Only defining what qualifies | Also define what disqualifies |
| **Missing fallback logic** | Breaks when info is missing | Always handle "what if this isn't here?" |
...
The testing section of this guide (Part 3) is actually the second half of the prompt engineering process. You're not just testing criteria—you're discovering where your prompts need refinement and iterating until they work consistently.
...
### Common Mistakes in Extraction Instructions

| Mistake | Example | Fix |
|---------|---------|-----|
| **Including threshold** | "Extract AHI if ≥15" | "Extract AHI value from sleep study" |
| **Including qualification logic** | "Find face-to-face visit within 6 months" | "Find face-to-face visit date" + separate criterion for 6-month rule |
| **Making comparisons** | "Find blood pressure over 140/90" | "Find blood pressure readings" + separate criterion for threshold |
| **Including exclusions** | "Get diagnosis but not if it's for comfort only" | "Get diagnosis" + separate criterion excluding comfort-only cases |
| **Combining multiple fields** | "Get age and verify over 3" | Create separate extractions for each piece of data |

### Why This Separation Matters for Compliance
...
## Part 3: The Formatting Problem

### Why Format Matters More Than You Think

Consider these two ways of writing the same requirement:

**Version A:**
```
Patient must meet applicable criteria for mobility limitation
```

**Version B:**
```
The patient must have a mobility limitation that impacts their ability to 
perform at least one Mobility-Related Activity of Daily Living (MRADL) 
within the home, in the patient's current unassisted state.

A qualifying mobility limitation meets at least ONE of the following:
A. The patient is unable to perform the MRADL without assistance
B. Attempting the MRADL without the equipment places the patient at 
   increased risk of illness, injury, or medical complications
C. The patient cannot complete the MRADL within a reasonable time frame 
   without the prescribed device
```

Version A is how a human might skim a policy. Version B is what an AI model needs to make consistent decisions.

**The difference:**
- Version A leaves "applicable criteria" undefined
- Version A doesn't specify what qualifies as a limitation
- Version A doesn't clarify the baseline state (with or without assistance?)
- Version A will produce inconsistent results

Version B works because it:
- Defines exactly what counts as qualifying evidence
- Specifies the evaluation context (unassisted state)
- Uses clear logic operators (ONE of the following)
- Anticipates ambiguity and resolves it

### The Three Formatting Principles

#### 1. Explicit Logic Structure
...
#### 2. Baseline State Specification
...
**Expected outcome:** You should find problems in 20-40% of cases on first run. This is normal.
...
### Mistake 1: Testing Only "Happy Path" Cases

**The mistake:** All your test packets are perfect examples with complete documentation, clear diagnoses, and obvious qualification.

**Why it fails:** Real world isn't like this. You need:
- Packets missing information
- Packets with ambiguous wording
- Packets from poor-quality scans
- Packets with information in unexpected locations
- Packets that SHOULD fail

**How to avoid:** Intentionally include problematic packets in your test set.

### Mistake 2: Changing Criteria Without Full Retesting

**The mistake:** You find a failure, fix that specific criterion, retest that one packet, and move on.

**Why it fails:** Your change might have unintended consequences. Fixing threshold logic for one criterion might affect how another criterion behaves.

**How to avoid:** After any change, retest your entire test set. Yes, all 30 packets. Every time.

### Mistake 3: Not Documenting Edge Case Decisions

**The mistake:** You encounter an ambiguous case, make a judgment call, and move on without documentation.

**Why it fails:** Next month someone asks "why did we handle it this way?" and no one remembers. Or you encounter a similar case and make a different decision, creating inconsistency.

**How to avoid:** Document every edge case decision:
- What was ambiguous
- What decision you made
- Why you made that decision
- What the alternative interpretations were

### Mistake 4: Ignoring Patterns in Failures

**The mistake:** You see 5 failures but treat each as a unique problem requiring a unique fix.

**Why it fails:** Often multiple failures share a root cause. If you fix them individually, you miss the pattern and will see similar failures in production.

**How to avoid:** After bulk testing, categorize failures:
- Extraction failures (couldn't find data): Does one extraction prompt need work?
- Logic failures (wrong decision): Does one criterion need clearer logic?
- Threshold failures (ambiguous near cutoff): Do you need to define edge case handling?
- Provider-specific failures: Do you need more variation handling?

Fix the pattern, not the individual cases.

### Mistake 5: Stopping at 95% Accuracy

**The mistake:** "We hit 95%, we're done!"

**Why it fails:** That remaining 5% might all be false positives (dangerous) or might all be from one provider type or might all involve one specific edge case.

**How to avoid:** Analyze what constitutes your 5% error rate. If it's random noise across all categories, you're probably done. If it's clustered, you have more work to do.
