...
Use when:
- Exact or near-exact match to `insurance_payer`, or
- Exact or near-exact match to `alias_name`, or
- Very clear abbreviation expansion with matching state/LOB, or
- Strong brand + state + LOB alignment with no serious competing candidates.
...
When evaluating candidates, follow this order:

### Step 1 — Detect placeholders first
If the input is self-pay, copay, cash, collections, payment method, no insurance, or other non-insurance placeholder, stop. Return no payer match.

### Step 2 — Look for exact `alias_name` match
If the input exactly or nearly exactly matches a CSV `alias_name`, prefer that row.

Reason: aliases are often created specifically to capture customer/EHR naming.

### Step 3 — Look for exact `insurance_payer` match
If the input exactly or nearly exactly matches `insurance_payer`, choose that row.

### Step 4 — Look for strong normalized match to either field
...
### Step 5 — Use taxonomy to disambiguate
...
Do not overuse state for national commercial payers when the plan is not state-specific. For example, `Aetna PPO` with a Texas address can still be generic Aetna commercial if there is no `Aetna Texas` row.

Be careful with two-letter state abbreviations that overlap with payer abbreviations or plan names:
- `MA` can mean Massachusetts or Medicare Advantage. In payer context, `MA plan`, `MA HMO`, or `MA-PD` means Medicare Advantage. In an address/state column, `MA` means Massachusetts.
- `IN` can mean Indiana or a word in a phrase.
- `OR` can mean Oregon or the word “or”.
...
- State is missing for a state-specific payer family like BCBS or Medicaid MCO
